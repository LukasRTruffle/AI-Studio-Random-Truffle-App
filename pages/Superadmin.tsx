...
const Superadmin: React.FC = () => {
  ...
  const currentTitle = superadminNavItems.find(item => item.path === location.pathname)?.name || 'Super Admin';
  return (
        ...
        <div className="flex flex-col lg:flex-row gap-6">
            <nav className="flex flex-row lg:flex-col lg:w-64 space-x-2 lg:space-x-0 lg:space-y-1 overflow-x-auto pb-2">
                {superadminNavItems.map(item => (
                    <NavLink key={item.path} to={item.path} end={item.path === '/superadmin'} className={({isActive}) => `flex items-center p-3 rounded-md text-sm whitespace-nowrap ${isActive ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-100'}`}>
